...
Plat-play "dsk" in (gmright2-pc):true this dsk is now on the run -buo
 Plat-check build $gmright2 is web/pro Quality version web build by gmright2 administration core command the Jekyll version ignore : ,code_line#51 and build your new path in Jekyll control command"true"
Plat_check ("x.code")ubuta-database start.analyzing ; gmright2 platform automatic -ignore/argument.ignore set up the extended of gmright2 application in platformions.wrote by gmright2 technology "true"
  Jekyll-build-deploy.server in https://GitHub.com
Jekyll-build-deploy/game/echo cardiogram off/configuration.on/platformions.action
Jekyll-build-parent-home/gmright2.output/operation-system;'optionL/yellow-color/files with the yellow color stick around and build new location
...
tomorrow we are going  going to working on publishing new parents application that would be able to help with user who wants to join gmright2 this data-sheet it would automatically be able to work in gmright2 this is because we want make sure everything is order it's important to keep everything stable 
